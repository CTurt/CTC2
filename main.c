...
int main(int argc, char **argv) {
	...
	if(argc < 2) {
		printf("CTC2 Compressor / Decompressor\n");
		printf("Usage:\n");
		printf("CTC2 input.bin [-c] [-d] [-o output.bin]\n");
		printf(" -c: Compress\n");
		printf(" -d: Decompress\n");
		...
		printf("EG: CTC2 data.bin -c -o data_compressed.bin\n");
		return 1;
	}
	...
	if(mode == MODE_COMPRESS) {
		...
		outputSize = CTC2_Compress(NULL, inputData, inputSize);
		
		outputData = malloc(outputSize);
		
		CTC2_Compress(outputData, inputData, inputSize);
		...
		outputSize = CTC2_Decompress(NULL, inputData, inputSize);
		
		outputData = malloc(outputSize);
		
		CTC2_Decompress(outputData, inputData, inputSize);
